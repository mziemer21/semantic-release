...
const getGitHead = require('git-head');
...
module.exports = async opts => {
  ...
  const nextRelease = {type, version, gitHead: await promisify(getGitHead)(), gitTag: `v${version}`};
  ...
  if (options.dryRun) {
    ...
    await plugins.publish({options, pkg, logger, lastRelease, commits, nextRelease}, async prevInput => {
      const newGitHead = await promisify(getGitHead)();
